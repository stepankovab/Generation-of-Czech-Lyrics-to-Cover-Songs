...
python $DATADIR/GPT2_baseline.py --model "tinyLlama" --dataset_path $DATADIR --dataset_type 1 --starting_epoch 3 >> $DATADIR/Out_file_ll1.out || { echo >&2 "Calculation ended up erroneously (with a code $?) !!"; exit 3; }

# move the output to user's DATADIR or exit in case of failure

python $DATADIR/GPT2_baseline_infer.py --model "tinyLlama" --dataset_path $DATADIR --dataset_type 1 >> $DATADIR/Out_file_ll1i.out || { echo >&2 "Calculation ended up erroneously (with a code $?) !!"; exit 3; }
