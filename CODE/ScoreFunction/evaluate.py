...
def score_function(czech_sections : list[list[str]], english_sections : list[list[str]], BERT = None) -> float:
    """
    ...
    semantic_similarity = get_semantic_similarity(czech_sections, english_sections, BERT)
...
filenames = os.listdir("DATA/Aligned_HT")

czech_og = []
counter = 0

BERT = SentenceTransformer('paraphrase-multilingual-MiniLM-L12-v2')  # multi-language model

for filename in filenames:

    og_sections = []
    with open("DATA/Aligned_HT/" + filename , "r", encoding="utf-8") as f:
        lines = f.readlines()
        temp = []
        for line in lines:
            line = line[:-1]
    ...
    if filename[-6:] == "en.txt":
        ...
        scores["en_to_cz"].append(score_function(czech_sections, og_sections, BERT))
        
        print("\n\n" + filename[:-7] + " - the original HT lyrics:")
        scores["HT"].append(score_function(czech_og, og_sections, BERT))
    else:
        print("\n\n" + filename[:-7] + " from czech to english:")
        english_sections = get_aligned_translation(og_sections, "en")
        scores["cz_to_en"].append(score_function(og_sections, english_sections, BERT))
