...
python $DATADIR/GPT2_baseline.py --model "tinyLlama" --dataset_path $DATADIR --dataset_type 9 >> $DATADIR/Out_file_ll9.out || { echo >&2 "Calculation ended up erroneously (with a code $?) !!"; exit 3; }

# move the output to user's DATADIR or exit in case of failure

python $DATADIR/GPT2_baseline_infer.py --model "tinyLlama" --dataset_path $DATADIR --dataset_type 9 >> $DATADIR/Out_file_ll9i.out || { echo >&2 "Calculation ended up erroneously (with a code $?) !!"; exit 3; }
