...
#PBS -l select=1:ncpus=1:ngpus=1:mem=20gb:scratch_local=100gb:gpu_cap=cuda86
#PBS -N tinyLlama_infer_baseline
...
python $DATADIR/GPT2_baseline_infer.py >Llama_baseline.out || { echo >&2 "Calculation ended up erroneously (with a code $?) !!"; exit 3; }
