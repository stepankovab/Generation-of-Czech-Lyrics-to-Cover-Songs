...
python $DATADIR/GPT2_baseline.py --model "GPT2_czech_XL" --dataset_path $DATADIR --dataset_type 7 >> $DATADIR/Out_file_czXL7.out || { echo >&2 "Calculation ended up erroneously (with a code $?) !!"; exit 3; }

# move the output to user's DATADIR or exit in case of failure

python $DATADIR/GPT2_baseline_infer.py --model "GPT2_czech_XL" --dataset_path $DATADIR --dataset_type 7 >> $DATADIR/Out_file_czXL7i.out || { echo >&2 "Calculation ended up erroneously (with a code $?) !!"; exit 3; }
